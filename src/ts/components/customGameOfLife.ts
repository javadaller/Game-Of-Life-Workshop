    ...
    while (localStorage.getItem('customRunning') === 'true') {
        ...
        customArray.forEach(cellID => {
            ...
            deadNeighbors.forEach(deadCellID => {
                ...
                const parents: number = getNeighbors(nx, ny, rows, cols, 'true').length
                if (parents === 3) {
...
function aliveCell(cell: string): void {
    const div: HTMLElement = document.getElementById(cell)
    if (div) {
...
function deadCell(cell: string): void {
    const div: HTMLElement = document.getElementById(cell)
    if (div) {
